...
<div class="dash-side-nav-wrapper" id="dash-side-menu">
    <div class="dash-side-nav perfect-scroll">
        ...
        <ul class="list-unstyled dash-menu">
            ...
            <li><a href="<%=basePath%>message/sent/0" class="current"><i class="fa fa-envelope-o"></i><span>已发送</span></a></li>
            ...
            <ul class="list-unstyled msg-items">
                <c:forEach var="item" items="${messages}" varStatus="cnt">
                    ...
                    <c:if test="${cnt.index==selectId}">
                        <a class="current" href="<%=basePath%>message/sent/${cnt.index}">
                            <strong>${item.receiver.userName}</strong>
                            <span>${item.books.name}</span>
                            <em>${item.message.time}</em>
                        </a>
                    </c:if>
                    <c:if test="${cnt.index!=selectId}">
                        <a href="<%=basePath%>message/sent/${cnt.index}">
                            <strong>${item.receiver.userName}</strong>
                            <span>${item.books.name}</span>
                            <em>${item.message.time}</em>
                        </a>
            ...
            <div class="msg">
                <c:if test="${!empty messages}">
                    <div class="sender-info">
                        ...
                        <p><strong>${messages.get(selectId).receiver.userName}</strong><span>${messages.get(selectId).message.time}</span></p>
                    </div>
